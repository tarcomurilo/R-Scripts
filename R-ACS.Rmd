...
calcTabelaFito <- function(DadosFito, Parametros){
  ...
  tabelaFito$DAP.Médio <- sapply(aggregate(DadosFito$DAP, by=list(DadosFito$Especie), FUN=mean)[2], as.numeric)
  tabelaFito$Altura.Média <- sapply(aggregate(DadosFito$Altura, by=list(DadosFito$Especie), FUN=mean)[2], as.numeric)
  tabelaFito$AB <- sapply(aggregate(DadosFito$AB, by=list(DadosFito$Especie), FUN=mean)[2], as.numeric)
  
  colnames(tabelaFito) <- c("Espécie", "NI", "NPO", "DAP. Médio", "Alt. Média", "AB")
  ...
  tabelaFito$"IVI %" <- tabelaFito$IVI / sum(tabelaFito$IVI)
...
Fitossociologia <- calcTabelaFito(DadosImportados, Parametros)
