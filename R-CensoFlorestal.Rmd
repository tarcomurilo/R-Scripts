...
ClasH <- data.frame("Classe" = c(paste(as.character(min(rdata$Altura))," - ",as.character(round(mean(rdata$Altura, na.rm=TRUE)-sd(rdata$Altura, na.rm=TRUE), 1)), sep=""),paste( as.character(round(mean(rdata$Altura, na.rm=TRUE)-sd(rdata$Altura, na.rm=TRUE), 1)), " - ", as.character( round( mean( rdata$Altura, na.rm=TRUE) + sd(rdata$Altura, na.rm=TRUE), 1)), sep="" ) ,paste( as.character( round( mean( rdata$Altura, na.rm=TRUE) + sd(rdata$Altura, na.rm=TRUE), 1))," - ", as.character(round(max(rdata$Altura, na.rm=TRUE),1)), sep="")))
...
grafico_estv <- function(X){
  ggplot(X, aes(c(x=1:nrow(X)),y=X[,1], fill=as.factor(X[,2])),) + geom_col() + scale_fill_manual(values=c("#779966","#88AA88", "#557755"), labels=c("Estrato inferior", "Estrato médio", "Estrato superior")) + geom_line(y=mean(X[,1])-sd(X[,1]) , color="#223344") + geom_line(y=mean(X[,1])+sd(X[,1]), color="#885555") + labs(title="Perfil Vertical da Floresta", x="Indivíduo", y="Altura", fill="Legenda") + theme(legend.position="bottom", panel.background = element_rect(fill="#DDEEDD", colour = "#CCCCDD"), plot.title = element_text(hjust=0.5))
}

png("grafPV.png", height = 400, width = 600) #salva o grafic
grafico_estv(tmpAlt)
...
openxlsx::insertImage(wb,"Graficos", "grafPV.png", startRow = 2, startCol = 18, width = 6, height=4)
